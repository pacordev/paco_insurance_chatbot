...
2. **Actually shipping it somewhere people can use it.** Landed on the shape of this: a FastAPI backend wrapping the existing `Dispatcher`/`TermStore` core, and a plain static HTML/CSS/JS frontend in front of it (no framework, no build step — this stage doesn't need one). Deliberately three separate repos rather than one — this repo (`ins_chatbot`) stays exactly as it is today, clonable and runnable from the command line with no changes, and gets pip-installed as a dependency (`pip install git+https://github.com/pacordev/paco_insurance_chatbot.git`) by a new API repo rather than merged into it. A third repo holds the frontend.
   - **API: built and deployed.** Lives in its own repo (`insurance_chatbot_api`), depends on this one as a normal pip package, and is live on Render (it needs to stay warm as a persistent process, since loading `TermStore`/spaCy at startup is the expensive part — measured at well under half a second in practice, so the real cost is Render's own container cold-start after ~15 minutes idle on the free tier, not this project's code). Three endpoints (session start, chat, health check), a random `session_id` per conversation, and the security approach below — all working, verified end to end against the live deployment, not just locally.
   - Worked through the security question properly before settling on an approach: the frontend will be public (URL just shared informally with coworkers, not gated), and nothing shipped in public client-side JS can be a real secret — a login form on a static site is visible in devtools regardless of how it's built. Landed on an API key (header-based) plus CORS plus rate limiting: enough to keep out bots/crawlers/casual traffic and to have a clean revocation lever, honestly scoped to what it actually protects rather than pretending to be real access control. **Implemented and live**, not just planned — confirmed the auth rejection and rate limiting both actually work against the real deployment. If this ever needs to be genuinely restricted to coworkers and no one else, real SSO/OAuth is the actual next step, not attempted yet.
   - **Frontend: built and live.** A plain static HTML/CSS/JS frontend (no framework, no build step) in its own repo, deployed on Vercel and talking to the live Render API — reaching the bot today means opening a browser, not calling curl or Postman. One real bug turned up through actual use rather than automated testing: after a long conversation, the newest reply sometimes didn't scroll into view, because the code re-scrolled the chat window when it inserted a "…" placeholder but never re-scrolled once the real (often longer) reply replaced it. One-line fix. Full technical plan and build details are in this project's internal `handoff.md` log.
...
- **Every definition in the dataset is now substantially rewritten by Claude, not just sourced from the original scraped content.** What started as a couple of Claude-authored content batches (new terms added from general knowledge, not from any source I provided) grew into something bigger: I asked for every definition to be expanded from a terse one-liner into a fuller 2-3 sentence explanation, across the entire dataset. The result reads much better, but it does mean the definitions themselves are one step further from the original source material than before — worth keeping in mind alongside the point above.
...
### Shipping the frontend, and a bug that only surfaced through real use

With the API live, the last piece of "access for the team" was actually getting it in front of a browser instead of curl or Postman. The frontend itself — a plain static site matching the visual style of my personal site, recolored blue — had already been built and verified locally in an earlier session; this one was about actually putting it somewhere real. A `.gitignore` and README first, the same pattern the API repo got before its own first commit, then a new GitHub repo — one false start along the way, a repo created with "api" in its name by mistake, caught and abandoned before anything got pushed to it — then a real `git init`, commit, and push.

Deploying to Vercel took one deliberate decision: since there's no build tooling here at all, generating the frontend's config file with the real API key at deploy time — instead of committing it — meant configuring a plain shell command as the "build" step, not the usual npm/yarn setup wizard. Vercel doesn't care that there's no `package.json`; a build command runs either way. One small but real gotcha wiring the two services together: CORS's `Origin` header is always just the scheme and host, never a trailing slash or path — an easy thing to get wrong when copy-pasting a URL out of a browser's address bar into an environment variable.

Once it was live, testing it for real turned up something the automated test suite never would have caught: after a longer conversation, the newest reply sometimes didn't scroll into view. The chat window auto-scrolled when it inserted a "…" placeholder for the bot's reply, but never re-scrolled once the real (often longer) reply replaced that placeholder's text. Invisible in a short test session, real once the window filled up. One line fixed it — a good reminder that this project's whole testing philosophy (run the thing, don't just trust that it should work) applies just as much to a browser UI as it does to the entity matcher back in the early days.

### Narrowing the dictionary to its real audience, and giving every definition a proper rewrite

Two content passes landed close together. First: a reminder that this bot is built for Latin America and Canada, not the US, surfaced a handful of US-only terms sitting in the dictionary — a couple of American federal/state laws, a US professional body's practice guidelines. Rather than mechanically strip every mention of the word "American" and leave whatever grammar was left behind, I split them into two groups: the ones entirely about a US-only law with nothing to generalize into got removed outright, and the ones where the underlying concept — foreign investment, the independent-agent distribution model, municipal bonds, actuarial qualification — was genuinely portable, just described using a US-specific label, got reworded instead.

Second, and much bigger: the definitions themselves had been bothering me for a while — most were a single terse sentence, technically correct but not much of an explanation for someone actually trying to learn the term. Rather than rewrite all 1,339 blind, I did a small pilot first: one term per category, each expanded into 2-3 sentences covering what it is, why it matters, and how it's actually used, and checked the style before committing to the rest.

Once the style was approved, scaling it to the whole dataset turned into its own small project — big enough that it ran as a background pass while I kept working on other things, and it got interrupted twice along the way (the laptop went to sleep mid-run once, and it hit a usage limit right before its very last batch). Both times it picked back up by checking the actual state of the file rather than trusting its own memory of where it had left off, which is exactly the instinct I'd want here — re-verify against reality instead of assuming. I did the same thing on my end once it reported finished: re-ran the test suite and re-checked the numbers myself rather than taking a "done" summary at face value.

The definitions read noticeably better now — real explanations instead of dictionary-terse one-liners — but it's worth being honest about what that means for the dataset's provenance: nearly all 1,339 definitions are now something Claude wrote, not the original scraped source text. That's a bigger version of a caveat this project has carried from early on — solid enough for informal, internal learning, not yet something to present as an authoritative source without a real review pass.
